...
import { eliminarTrimestre } from '../../../data/sendRequest';
...
export const ModalDelet = ({
  ...
  const handleDeletTrimestre = async () => {
    try{
      await eliminarTrimestre(id)
      handleClose()
      window.location.reload(); 
    } catch (error) {
      console.error("Error al eliminar el trimestre:", error.message);
    }
  }
  return (
    <React.Fragment>
      ...
      >
        <DialogContent>
          ...
          <h5 className='mt-3 font-semibold text-lg uppercase pb-2'>Esta usted seguro ?</h5>
            <p className='px-2 text-gray-400'>
             ¿Está seguro de eliminar al trimestre? Con esta acción se desactivarán los datos del mismo y toda su información relacionada.  
            </p>
          </DialogContentText>
        </DialogContent>
        <Divider/>
        <DialogActions>
          ...
          <button 
            ...
            onClick={handleDeletTrimestre}>Eliminar</button>
        </DialogActions>
      </Dialog>
    </React.Fragment>
  );
